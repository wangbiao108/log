                   ...
                   std::string       file_name)
{
    ...
    if (start_id_file.is_open()) {
        ...
        while (std::getline(start_id_file, line)) {
            printf("<read_start_ids>: line: %s\n", line.c_str());
            std::stringstream lineStream(line);
            std::string       vals;
            int               i1 = 0;
            std::vector<int>  tmp_vec;
            while (std::getline(lineStream, vals, ',')) {
                printf("<read_start_ids>: i1:%d: %s\n", std::stoi(vals));
...
std::map<std::string, std::pair<int, int>> init_prompt_tuning_map(const INIReader& reader, const model_config_t& config)
{
    ...
    for (int task_name_id = 0; task_name_id < config.prompt_learning_num_tasks; task_name_id++) {
        ...
        printf("init_prompt_tuning_map: {task_name:%s, {task_name_id:%d, prompt_length:%d} config_task_name:%s\n", 
            task_name.c_str(), task_name_id, prompt_length, config_task_name);
...
std::pair<int, int> init_multiprocessing(const model_config_t& model_config)
{
    ...
    printf("init_multiprocessing rank: %d\n", rank);
    ...
    if (model_config.tensor_para_size * model_config.pipeline_para_size != world_size) {
        printf("[ERROR] tensor_para_size[%d] * pipeline_para_size[%d] should equal to world_size[%d] \n",
...
{
    int device=0, device_count=2;
    printf("《init_cuda_ctx》rank: %d\n", rank);
    check_cuda_error(cudaGetDeviceCount(&device_count));
    printf("device_count: %d\n", device_count);
    if (device_count!=0) {
        printf("device_count: %d, rank: %d, current device:%d\n", device_count, rank, rank % device_count);
    }
...
void write_output_tensors(std::unordered_map<std::string, Tensor>& output_tensors)
{
    ...
    if (!out_file.is_open()) {
        ...
        std::cout << "out_count " << out_count << " total_output_len " << total_output_len << "\n";
