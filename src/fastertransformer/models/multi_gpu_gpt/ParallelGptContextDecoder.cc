    ...
    const std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    printf("[%s:%d-pid:%d-%s]: <......begin.....>  \n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__);

    PUSH_RANGE("buffer allocation");
    printf("[%s:%d-pid:%d-%s]: <allocateBuffer> [buffer allocation] seq_len=%d batch_size=%d use_shared_contexts=%d\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        seq_len, batch_size,use_shared_contexts);
    allocateBuffer(batch_size, seq_len, use_shared_contexts);
    POP_RANGE;

    PUSH_RANGE("compact inputs");
    if (use_shared_contexts) {
        printf("[%s:%d-pid:%d-%s]: <invokeCompactInputs> [compact inputs] seq_len=%d batch_size=%d hidden_units_=%d\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            seq_len, batch_size,hidden_units_);
    ...
    printf("[%s:%d-pid:%d-%s]: <-----loop iteration_num[%d]------> [context_generation]\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__, iteration_num);
    for (uint ite = 0; ite < iteration_num; ite++) {
        size_t h_token_num = local_batch_size * seq_len;
        printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> [context_generation]\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num);
        if (is_unpadded_mha) {
            const int* base_input_lengths =
                (use_shared_contexts ? compact_input_lengths_ : input_tensors->at("input_lengths").getPtr<int>());
            printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <invokeGetPaddingOffsetAndCuSeqLens> [context_generation] h_token_num=%d[%d,%d] size:%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num,
                h_token_num, local_batch_size, seq_len,input_tensors->at("input_lengths").size());

            {
                int out_size = batch_size * seq_len;
                printf("-----------------padding_offset_ [invokeGetPaddingOffsetAndCuSeqLens] pre padding_offset_size:%d[%d,%d]\n", 
                    out_size, batch_size, seq_len);
                std::vector<int> h_buf(out_size);
                cudaD2Hcpy(h_buf.data(), padding_offset_, out_size);
                printf("-----------------padding_offset_ [invokeGetPaddingOffsetAndCuSeqLens] pre padding_offset_:\n");
            ...
            invokeGetPaddingOffsetAndCuSeqLens(h_pinned_token_num_ptr_,//host端内存 h_pinned_token_num_ptr_[0]=8   输出
                                               &h_token_num,//host端变量 token个数 8  输出
                                               padding_offset_, //device pad偏移  【batch,seglen] [1,8]  输出
                                               cu_seqlens_, //device   [batch+1]   输出
                                               base_input_lengths + ite * local_batch_size, //输入：input_lengths+batch偏移  输入
                                               local_batch_size,
                                               seq_len,
                                               stream_);
             printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <invokeGetPaddingOffsetAndCuSeqLens> end [context_generation] h_token_num=%d h_pinned_token_num_ptr_[0]=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num,
                h_token_num, (int32_t)h_pinned_token_num_ptr_[0]);

            {
                int out_size = batch_size+1;
                printf("-----------------cu_seqlens_ [invokeGetPaddingOffsetAndCuSeqLens] cu_seqlens_size:%d[%d,%d]\n", 
                    out_size, batch_size, seq_len);
                std::vector<int> h_buf(out_size);
                cudaD2Hcpy(h_buf.data(), cu_seqlens_, out_size);
                printf("-----------------cu_seqlens_ [invokeGetPaddingOffsetAndCuSeqLens] cu_seqlens_:\n");
                for(int k=0; k<out_size; k++) {
                    printf("%d ", h_buf[k]);
                } 
                printf("\n");
            ...
            {
                int out_size = batch_size * seq_len;
                printf("-----------------padding_offset_ [invokeGetPaddingOffsetAndCuSeqLens] padding_offset_size:%d[%d,%d]\n", 
                    out_size, batch_size, seq_len);
                std::vector<int> h_buf(out_size);
                cudaD2Hcpy(h_buf.data(), padding_offset_, out_size);
                printf("-----------------padding_offset_ [invokeGetPaddingOffsetAndCuSeqLens] padding_offset_buf:\n");
                for(int k=0; k<out_size; k++) {
                    printf("%d ", h_buf[k]);
                } 
                printf("\n");
        ...
        printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <---loop num_layer_[%d]---> [context_generation]\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, num_layer_);
        for (uint l = 0; l < num_layer_; l++) {//decoder layer loop
            printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> [context_generation] pipeline-rank:%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
            ...
            if (isValidLayerParallelId(l) == false) {
                //pipeline切分时跳过不处理layer层
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> [跳过] pipeline-rank:%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                    pipeline_para_.rank_);
                continue;
            }
            ...
            if (l == 0 && is_unpadded_mha) {
                ...
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeRemovePadding> [remove padding] h_token_num=%d, hidden_units_=%d seq_len=%d use_shared_contexts=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                    h_token_num, hidden_units_, seq_len, use_shared_contexts);
                invokeRemovePadding(decoder_layer_output_,
                                    base_input + ite * local_batch_size * seq_len * hidden_units_,
                                    padding_offset_,
                                    h_token_num,//seq*batch_size
                                    hidden_units_,//1024
                                    stream_);
                POP_RANGE;
            }
            ...
            if (isFirstLayerParallelId(l) //l属于当前pipeline切分 判断当前l层是不是属于第一个layer并行块的id
                && pipeline_para_.rank_ != 0) {
                // 非0(pipeline-rank)接收layer数据
                PUSH_RANGE("input communication");
                const int data_size = h_token_num * hidden_units_ / tensor_para_.world_size_;
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <----ftNcclRecv-----> [非pipeline-rank等待前面layer输入数据] pipeline_para_.rank_=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                    pipeline_para_.rank_);
                ...
                if (tensor_para_.world_size_ > 1) {
                    ...
                    printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <-----ftNcclAllGather----> [all gather] pipeline_para_.rank_=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_, pipeline_para_.rank_);
            ...
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeGeneralLayerNorm> [pre-mha layernorm]\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
                invokeGeneralLayerNorm(decoder_normed_input_,//out  [batch_size, seq_len, hidden_unit]
                                       decoder_input,//in [batch_size, seq_len, hidden_unit]
                                       layer_weight->pre_layernorm_weights.gamma,//in
                                       layer_weight->pre_layernorm_weights.beta,//in
                                       layernorm_eps_,//in
                                       h_token_num,  // batch_size*seq_len=1*8=8
                                       hidden_units_,//1024
            ...
            printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <self_attention_layer_->forward> []\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
            ...
            if (use_shared_contexts) {
                ...
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeUnCompactCaches> [K/V cache clean]\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
            ...
            if (has_adapters_) {
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeGenericActivation> [post mha layernorm]\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
                ...
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <ffn_layer_->forward> []\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
            ...
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeGeneralAddBiasResidualPreLayerNorm> [] has_adapters_=%d decoder_input=%p self_attn_output_=%p\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, 
                    ite, iteration_num, l, num_layer_,has_adapters_,decoder_input,self_attn_output_);
                invokeGeneralAddBiasResidualPreLayerNorm(
                    has_adapters_ ? after_adapter_attn_output_ : self_attn_output_,//out has_adapters_=0
                    normed_self_attn_output_,//out
                    has_adapters_ ? after_adapter_attn_output_ : self_attn_output_,//in
                    decoder_input,//in 残差1
                    has_adapters_ ? self_attn_output_ : nullptr,// 残差2
                    layer_weight->self_attn_layernorm_weights.gamma,//gamma
                    layer_weight->self_attn_layernorm_weights.beta,//beta
                    has_adapters_ ? layer_weight->after_attention_adapter_weights.output_weight.bias :
                                    layer_weight->self_attention_weights.attention_output_weight.bias,//bias
                    layernorm_eps_,
                    h_token_num,
                    hidden_units_,
                    nullptr,
                ...
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeAddBiasResidualLayerNorm> []\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
            ...
            if (has_adapters_) {
                ...
                TensorMap ffn_output_tensors;
                if (!use_moe) {
                    printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeGenericActivation> [post ffn] has_adapters_=%d use_moe=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        has_adapters_,use_moe);
                ...
                ffn_layer_->resetInterSize(adapter_inter_size_ / tensor_para_.world_size_);
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <ffn_layer_->forward> [post ffn]\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_);
            ...
            if (!use_moe) {
                if (layernorm_type_ == LayerNormType::pre_layernorm) {
                    printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeAddBiasResidual> [post ffn] layernorm_type_=%d use_moe=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        layernorm_type_,use_moe);
                    ...
                    printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeAddBiasResidualLayerNorm> [post ffn] layernorm_type_=%d use_moe=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        layernorm_type_,use_moe);
                ...
                if (layernorm_type_ == LayerNormType::pre_layernorm) {
                    printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <finalize_moe_routing_kernelLauncher> [post ffn] layernorm_type_=%d use_moe=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        layernorm_type_,use_moe);
                    ...
                    printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeGeneralLayerNorm> [post ffn] layernorm_type_=%d use_moe=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        layernorm_type_,use_moe);
            ...
            if (isLastLayerParallelId(l) == true &&  //每个pipeline切分layer的最后一层判定
                (pipeline_para_.rank_ != pipeline_para_.world_size_ - 1)) { //当前pipeline切分非最后一个处理块
                const int data_size = h_token_num * hidden_units_ / tensor_para_.world_size_;
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <ftNcclSend> [Nccl send] 【当前pipeline切分处理的最后一层layer且该切分非最后一块进行数据发送】 pipeline_rank=%d, world-size=%d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        pipeline_para_.rank_,pipeline_para_.world_size_);
            ...
            if ((l == num_layer_ - 1) && is_unpadded_mha) {
                ...
                printf("[%s:%d-pid:%d-%s]: <ite:%d/%d]> <layer_%d/%d]> <invokeRebuildPadding> [Rebuild padding] %d h_token_num=%d %d\n", 
                        __FUNCTION__, __LINE__, getpid(),__FILENAME__, ite, iteration_num, l, num_layer_,
                        ite * local_batch_size * seq_len * hidden_units_,//0
                        h_token_num,//8
                        head_num_ * size_per_head_);//1024=16*64
                invokeRebuildPadding(base_ptr + ite * local_batch_size * seq_len * hidden_units_,//out
                                     decoder_layer_output_,//in
                                     padding_offset_,//in
                                     h_token_num,
            ...
            POP_RANGE;
        }//for (uint l = 0; l < num_layer_; l++)结束
    }//for (uint ite = 0; ite < iteration_num; ite++)结束
    POP_RANGE;

    PUSH_RANGE("uncompact outputs");
    if (use_shared_contexts) {
        printf("[%s:%d-pid:%d-%s]: <invokeUnCompactOutputs> [uncompact outputs]\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__);
    ...
    printf("[%s:%d-pid:%d-%s]: <invokeLookupHiddenStateOfLastToken> [last token hidden state lookup] seq_len=%d request_batch_size=%d hidden_units_=%d last_token_hidden_units size: %d\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            seq_len, request_batch_size, hidden_units_,
            output_tensors->at("last_token_hidden_units").size());

    {
        //param.step_ids sequence_lengths_
        int out_size = hidden_units_;
        std::vector<T> h_buf(out_size);
        cudaD2Hcpy(h_buf.data(), output_tensors->at("last_token_hidden_units").getPtr<T>(), out_size);
        printf("-----------------last_token_hidden_units [invokeLookupHiddenStateOfLastToken] pre h_buf:\n");
        for(int k=0; k<out_size; k++) {
            printf("%d ", h_buf[k]);
        } 
        printf("\n");
    ...
    invokeLookupHiddenStateOfLastToken(output_tensors->at("last_token_hidden_units").getPtr<T>(), //输出
                                       output_tensors->at("decoder_output").getPtr<T>(),//输入
                                       input_tensors->at("input_lengths").getPtr<int>(),//输入
                                       seq_len,
                                       request_batch_size,
                                       hidden_units_,
                                       stream_);
    {
        ...
        std::vector<T> h_buf(out_size);
        cudaD2Hcpy(h_buf.data(), output_tensors->at("last_token_hidden_units").getPtr<T>(), out_size);
        printf("-----------------last_token_hidden_units [invokeLookupHiddenStateOfLastToken] end h_buf:\n");
        for(int k=0; k<out_size; k++) {
            printf("%d ", h_buf[k]);
        } 
        printf("\n");
