                                    ...
                                    const std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    printf("[%s:%d-pid:%d-%s]: <--------loop--------> num_layer_=%d \n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        num_layer_);
    for (uint l = 0; l < num_layer_; l++) {
        printf("[%s:%d-pid:%d-%s]: <layer_%d/%> \n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            l, num_layer_);
        ...
        if (isFirstLayerParallelId(l) == true && pipeline_para_.rank_ != 0 && pipeline_para_.world_size_ > 1) {
            ...
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <ftNcclRecv> [input communication]\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
        ...
        if (layernorm_type_ == LayerNormType::pre_layernorm) {
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeGeneralLayerNorm> [pre-mha layernorm] local_batch_size=%d hidden_units_=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_,
        ...
        printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <self_attention_layer_->forward>\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
        ...
        if (has_adapters_) {
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeGenericActivation> [post mha layernorm]\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
            ...
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <ffn_layer_->resetInterSize> []\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
            ffn_layer_->resetInterSize(adapter_inter_size_ / tensor_para_.world_size_);
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <ffn_layer_->forward> []\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
            ffn_layer_->forward(
        ...
        if (layernorm_type_ == LayerNormType::pre_layernorm) {
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeGeneralAddBiasResidualPreLayerNorm> []\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
            ...
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeAddBiasResidualLayerNorm> []\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_);
        ...
        if (has_adapters_) {
            ...
            TensorMap ffn_output_tensors;
            if (!use_moe) {
                printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeGenericActivation> [post ffn] use_moe=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    l, num_layer_,
                    use_moe);
        ...
        if (!use_moe) {
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeAddBiasResidual> [] use_moe=%d, layernorm_type_=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    l, num_layer_,
                    use_moe,layernorm_type_);
                ...
                printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeAddBiasResidualLayerNorm> [] use_moe=%d, layernorm_type_=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    l, num_layer_,
                    use_moe,layernorm_type_);
            ...
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <finalize_moe_routing_kernelLauncher> [] use_moe=%d, layernorm_type_=%d 带ffn_output_ptr\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    l, num_layer_,
                    use_moe,layernorm_type_);
                ...
                printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <finalize_moe_routing_kernelLauncher> [] use_moe=%d, layernorm_type_=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    l, num_layer_,
                    use_moe,layernorm_type_);
                finalize_moe_routing_kernelLauncher(fc2_result_,
                                                    ...
                                                    moe_k_,
                                                    stream_);
                printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <invokeGeneralLayerNorm> [] use_moe=%d, layernorm_type_=%d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    l, num_layer_,
                    use_moe,layernorm_type_);
        ...
        if (isLastLayerParallelId(l) == true && pipeline_para_.rank_ != pipeline_para_.world_size_ - 1
            ...
            printf("[%s:%d-pid:%d-%s]: <layer_%d/%> <ftNcclSend> [Nccl send] use_moe=%d, layernorm_type_=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                l, num_layer_,
                use_moe,layernorm_type_);
        ...
        POP_RANGE;
        POP_RANGE;
    } //for (uint l = 0; l < num_layer_; l++) loop结束

    //printf("end line:%d  pid:%d\n", __LINE__, getpid());
