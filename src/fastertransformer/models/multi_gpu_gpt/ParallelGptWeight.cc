...
{
    printf("[%s:%d-pid:%d-%s]: ==== <... begin ...> hidden_units_=%d int8_mode_=%d inter_size_=%d tensor_para_size_=%d tensor_para_rank_=%d gpt_variant_params_{%d,%d,%d,%d,%d,%d,%d] prompt_learning_pair_=%d,prompt_learning_type_=%d,layer_para_rank_=%d,layer_para_size_=%d,max_seq_len_=%d,num_layer_=%d,vocab_size_=%d\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        hidden_units_, int8_mode_, inter_size_, tensor_para_size_, tensor_para_rank_, 
        (int32_t)gpt_variant_params_.layernorm_type,
        (int32_t)gpt_variant_params_.activation_type,
        gpt_variant_params_.has_positional_encoding,
        gpt_variant_params_.has_pre_decoder_layernorm,
        gpt_variant_params_.has_post_decoder_layernorm,
        gpt_variant_params_.has_adapters,
        gpt_variant_params_.use_attention_linear_bias,
        prompt_learning_pair_,prompt_learning_type_,layer_para_rank_,
        layer_para_size_,max_seq_len_,num_layer_,vocab_size_);
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            printf("[%s:%d-pid:%d-%s]: <... load %d/%d layers model ...> \n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,l,num_layer_);
    ...
    printf("[%s:%d-pid:%d-%s]: ==== <... call mallocWeights ...> \n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__);
    mallocWeights();
    printf("[%s:%d-pid:%d-%s]: ==== <... call setWeightPtr ...> \n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__);
    setWeightPtr();
    printf("[%s:%d-pid:%d-%s]: ==== <... end ...> \n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__);
}
...
{
    printf("[%s:%d-pid:%d-%s]: <... begin ...> hidden_units_=%d int8_mode_=%d inter_size_=%d tensor_para_size_=%d tensor_para_rank_=%d gpt_variant_params_{%d,%d,%d,%d,%d,%d,%d] prompt_learning_pair_=%d,prompt_learning_type_=%d,layer_para_rank_=%d,layer_para_size_=%d,max_seq_len_=%d,num_layer_=%d,vocab_size_=%d %d %d %d\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        hidden_units_, int8_mode_, inter_size_, tensor_para_size_, tensor_para_rank_, 
        (int32_t)gpt_variant_params_.layernorm_type,
        (int32_t)gpt_variant_params_.activation_type,
        ...
        gpt_variant_params_.has_adapters,
        gpt_variant_params_.use_attention_linear_bias,
        prompt_learning_pair_,prompt_learning_type_,layer_para_rank_,layer_para_size_,max_seq_len_,num_layer_,vocab_size_,
        prompt_token_weight_size_,malloc_load_prompt_weights_,shared_embed_);
    mallocWeights();
...
{
    printf("[%s:%d-pid:%d-%s]: <... begin ...> 配置编码和位置嵌入权重，解码前后处理权重\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__);

    prompt_learning_table.resize(prompt_learning_pair_.size());

    position_encoding_table      = gpt_variant_params_.has_positional_encoding ? weights_ptr[0] : nullptr; // possition-table---->weights_ptr[0]
    pre_decoder_embedding_table  = weights_ptr[1];// embedding_table---->weights_ptr[1]
    pre_decoder_layernorm.gamma  = gpt_variant_params_.has_pre_decoder_layernorm ? weights_ptr[2] : nullptr;//pre_decoder_layernorm.gamma---->weights_ptr[2]  null
    pre_decoder_layernorm.beta   = gpt_variant_params_.has_pre_decoder_layernorm ? weights_ptr[3] : nullptr;//pre_decoder_layernorm.beta---->weights_ptr[3]   null
    post_decoder_layernorm.beta  = gpt_variant_params_.has_post_decoder_layernorm ? weights_ptr[4] : nullptr;//post_decoder_layernorm.beta---->weights_ptr[4]
    post_decoder_layernorm.gamma = gpt_variant_params_.has_post_decoder_layernorm ? weights_ptr[5] : nullptr;//post_decoder_layernorm.gamma---->weights_ptr[5]
    
    printf("[%s:%d-pid:%d-%s]: <... 0~5 ...> position_encoding_table=%p pre_decoder_embedding_table=%p %p %p %p %p\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        position_encoding_table,
        pre_decoder_embedding_table,
        pre_decoder_layernorm.gamma,
        pre_decoder_layernorm.beta,
        post_decoder_layernorm.beta,
        post_decoder_layernorm.gamma);
...
void ParallelGptWeight<T>::mallocWeights()
{
    ...
    if (gpt_variant_params_.has_positional_encoding) {
        deviceMalloc(&weights_ptr[0], max_seq_len_ * vocab_size_);//max_seq_len_ 8;  vocab_size_ 50304
    }
    // word embedding table.
    deviceMalloc(&weights_ptr[1], vocab_size_ * hidden_units_);//vocab_size_ 50304;  hidden_units_ 1024
...
{
    printf("[%s:%d-pid:%d-%s]: <... begin ...>  加载编码、位置嵌入及解码前后处理权重%s\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,dir_path.c_str());
    ...
    if (gpt_variant_params_.has_positional_encoding) {
        ...
        printf("[%s:%d-pid:%d-%s]: <...  ...>  shape[%d,%d] weights_ptr[0]=%p\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            max_seq_len_, hidden_units_,weights_ptr[0]);
    }
    loadWeightFromBin<T>(weights_ptr[1], {vocab_size_ * hidden_units_}, dir_path + "/model.wte.bin", model_file_type);
    printf("[%s:%d-pid:%d-%s]: <...  ...>  shape[%d,%d] weights_ptr[1]=%p\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            vocab_size_, hidden_units_,weights_ptr[1]);
    if (gpt_variant_params_.has_pre_decoder_layernorm) {
        loadWeightFromBin<T>(
            weights_ptr[2], {hidden_units_}, dir_path + "/model.pre_decoder_layernorm.weight.bin", model_file_type);
        printf("[%s:%d-pid:%d-%s]: <...  ...>  shape[%d] weights_ptr[2]=%p\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            hidden_units_,weights_ptr[2]);
        loadWeightFromBin<T>(
            weights_ptr[3], {hidden_units_}, dir_path + "/model.pre_decoder_layernorm.bias.bin", model_file_type);
        printf("[%s:%d-pid:%d-%s]: <...  ...>  shape[%d] weights_ptr[3]=%p\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            hidden_units_,weights_ptr[3]);
    }
    if (gpt_variant_params_.has_post_decoder_layernorm) {
        loadWeightFromBin<T>(
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            printf("[%s:%d-pid:%d-%s]: <... call  decoder_layer_weights[l]->loadModel ...>  加载layer:%d/%d权重\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,l,num_layer_);
...
template<typename T>
bool ParallelGptWeight<T>::isValidLayerParallelId(int l)//pipeline并行
{
