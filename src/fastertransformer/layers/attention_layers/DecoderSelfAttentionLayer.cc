                                           ...
                                           const AttentionWeight<T>* attention_weights)
{
    ...
    // /home/wangbiao/FasterTransformer/src/fastertransformer/layers/attention_layers/DecoderSelfAttentionLayer.cc
    ...
    printf("[%s:%d-pid:%d-%s]: <......begin.....> use_sparse_gemm=%d int8_mode_=%d memory_max_len=%d, batch_size=%d local_hidden_units_=%d input_query-size:%d\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        use_sparse_gemm, int8_mode_, memory_max_len, batch_size,
        local_hidden_units_,
        input_tensors->at("input_query").size());
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1) {
            ...
            printf("[%s:%d-pid:%d-%s]: <cublas_wrapper_->Gemm> m%dn%dk%d\n",  
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                batch_size, 3 * local_hidden_units_,d_model_);
            cublas_wrapper_->Gemm(CUBLAS_OP_N,
                                  CUBLAS_OP_N,
                                  3 * local_hidden_units_,  // n=3072
                                  batch_size,//   m=1
                                  d_model_,  // k  1024
                                  attention_weights->query_weight.kernel,//A
    ...
    POP_RANGE;
    printf("[%s:%d-pid:%d-%s]: <fusedQKV_masked_attention_dispatch> \n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__);
    fusedQKV_masked_attention_dispatch<T>(
        qkv_buf_,
        attention_weights->query_weight.bias,
        relative_attention_bias,
        key_cache,//output_tensors->getPtr<T>("key_cache")
        value_cache,//output_tensors->getPtr<T>("value_cache")
        cache_indir,
        context_buf_,
        finished,
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1) {
            ...
            printf("[%s:%d-pid:%d-%s]: <cublas_wrapper_->Gemm> m%dn%dk%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                batch_size,d_model_,local_hidden_units_);//m1n1024k1024
            cublas_wrapper_->Gemm(CUBLAS_OP_N, //transA 
                                  CUBLAS_OP_N, //transB
                                  d_model_,  // n 1024  //m
                                  batch_size, // 1      //n
                                  local_hidden_units_,  // k 1024 //k
                                  attention_weights->attention_output_weight.kernel, //A [1024*1024]
                                  d_model_,  // n  //lda [1024]
                                  context_buf_, //B  [1*1024] batch*local_hidden_units_   local_hidden_units_=local_head_num_ * size_per_head_
                                  local_hidden_units_,  // k //ldb [1024]
                                  attention_out,  //C   output_tensors->getPtr<T>("hidden_features");  [1*1024]
                                  d_model_ /* n */); //ldc [1024]
        }
        sync_check_cuda_error();
    }
