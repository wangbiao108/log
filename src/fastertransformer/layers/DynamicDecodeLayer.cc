...
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    printf("-----------vocab_size_=%d,vocab_size_padded_=%d\n", vocab_size_,
        vocab_size_padded_);
...
void DynamicDecodeLayer<T>::setup(const size_t batch_size, const size_t beam_width, TensorMap* runtime_args)
{
    ...
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    has_diff_runtime_args_ = hasDiffRuntimeArgs(runtime_args);//has_diff_runtime_args_在beam_width大于1时使用
...
void DynamicDecodeLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors)
{
    ...
    if (input_tensors->isExist("bad_words_list")) {
        ...
        printf("[%s:%d-pid:%d-%s]: <invokeBanBadWords> [Add check on batch size of bad words] shared_bad_words=%d bad_words_len=%d batch_size=%d/%d beam_width=%d id_offset=%d vocab_size_padded_=%d step=%d\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            shared_bad_words, (int32_t)bad_words_len, batch_size, local_batch_size,
            beam_width, id_offset, vocab_size_padded_, step);
    ...
    if (beam_width > 1) {
        ...
        printf("[%s:%d-pid:%d-%s]: <---loop---[%d,%d)> [dynamic decode GPT] \n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            ite * dynamic_decode_total_iteration,
            (ite + 1) * dynamic_decode_total_iteration);
        ...
        for (uint dynamic_ite = ite * dynamic_decode_total_iteration;
            ...
            if (true || beam_width < 16
                ...
                printf("[%s:%d-pid:%d-%s]: <online_beamsearch_decode_->forward> [only online_beamsearch_decode_ support beam_search_diversity_rate when beam_hyps is used] \n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__);
                ...
                printf("[%s:%d-pid:%d-%s]: <beamsearch_decode_->forward> [deprecate this module] \n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__);
        ...
        printf("[%s:%d-pid:%d-%s]: <in> logits.slice[{%d %d %d} %d] end_id.slice({%d},%d) step:%d max_input_length=%d ite=%d\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            local_batch_size, beam_width, logits.shape[2], 
            local_batch_offset * logits.shape[2],
            local_batch_size, ite * local_batch_size,
            input_tensors->at("step").getVal<int>(),
        ...
        // where "x" are skipped.
        printf("[%s:%d-pid:%d-%s]: <topk_decode_->forward> [] \n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__);
...

        {
            ...
            // printf("topk_decode_->forward param.step_ids[output_ids_buf_] h_buf:\n");
        ...
        printf("[%s:%d-pid:%d-%s]: <topp_decode_->forward> [] \n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__);
...

        {
            ...
            // std::vector<int> h_buf(out_size);
            // cudaD2Hcpy(h_buf.data(), output_ids_buf_, out_size);
            // printf("topp_decode_->forward param.step_ids[output_ids_buf_] h_buf:\n");
            // for(int k=0; k<out_size; k++) {
            //     printf("%d ", h_buf[k]);
            // }
            // printf("\n");
    ...
    if (input_tensors->isExist("stop_words_list")) {
        ...
        printf("[%s:%d-pid:%d-%s]: <invokeStopWordsCriterion> [] \n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__);
    ...
    if (input_tensors->isExist("sequence_limit_length")) {
        printf("[%s:%d-pid:%d-%s]: <invokeLengthCriterion> [] \n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__);
...
bool DynamicDecodeLayer<T>::hasDiffRuntimeArgs(TensorMap* input_tensors)
{
    for (int i = 0; i < (int)runtime_arg_names_.size(); i++) {
        printf("hasDiffRuntimeArgs: runtime_arg_names—_[%d]=%s \n", i, runtime_arg_names_[i].c_str());
        if (input_tensors->isExist(runtime_arg_names_[i])) {
            ...
            printf("hasDiffRuntimeArgs: runtime_arg_names—_[%d]=%s tensor.shape[0]=%d\n", 
                i, runtime_arg_names_[i].c_str(), (int)tensor.shape[0]);
