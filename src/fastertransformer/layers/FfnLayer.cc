...
void FfnLayer<T>::forward(TensorMap* output_tensors, TensorMap* input_tensors, const FfnWeight<T>* ffn_weights)
{
    ...
    printf("[%s:%d-pid:%d-%s]: <......begin.....> use_moe=%d\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,use_moe);
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1) {
            ...
            printf("[%s:%d-pid:%d-%s]: <......FFN gemm 1.....> use_gated_activation=%d inter_size_:%d/%d/%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,use_gated_activation,inter_size_,m,hidden_units_);
            cublas_wrapper_->Gemm(CUBLAS_OP_N,
                                  CUBLAS_OP_N,
                                  inter_size_,
                                  m,
    ...
    if (int8_mode_ != 1 || ia3_tasks != nullptr || use_gated_activation) {
        ...
        printf("[%s:%d-pid:%d-%s]: <......add bias act.....> [genericActivation] m=%d use_gated_activation=%d int8_mode_=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,m,use_gated_activation,int8_mode_);
        genericActivation(m,//8
                          ffn_weights->intermediate_weight.bias,
                          use_gated_activation ? ffn_weights->intermediate_weight2.bias : nullptr,//0
                          input_tensors->at("ia3_tasks", {MEMORY_GPU, TYPE_INT32, {}, nullptr}).getPtr<const int>(),
                          ffn_weights->ia3_weight.kernel,
                          int8_mode_ == 2 ? ffn_weights->intermediate_weight.scale_out : (float*)nullptr,//0
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1) {
            ...
            printf("[%s:%d-pid:%d-%s]: <......FFN gemm 2.....> [cublas_wrapper_->Gemm] m=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,m);
            cublas_wrapper_->Gemm(CUBLAS_OP_N,
                                  CUBLAS_OP_N,
                                  hidden_units_,
                                  m,
                                  ...
                                  ffn_weights->output_weight.kernel,
                                  hidden_units_,
                                  inter_buf_,  // [token_num,max_inter_size_]
                                  inter_size_,
                                  output_tensor,// ffn_output [token_num, hidden_dimension]
                                  hidden_units_);
        }
    }
    sync_check_cuda_error();
...
void FfnLayer<T>::allocateBuffer(size_t token_num, int moe_k, bool use_moe)
{
    ...
    if (use_moe) {
        ...
        printf("[%s:%d-pid:%d-%s]: <......inter_buf_.....> type_size=%d token_num=%d max_inter_size_=%d\n", 
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            type_size,token_num,max_inter_size_);
        if (use_gated_activation_) {
            inter_buf_2_ = (T*)allocator_->reMalloc(inter_buf_2_, sizeof(T) * token_num * max_inter_size_, false);
            printf("[%s:%d-pid:%d-%s]: <......inter_buf_2_.....> sizeof(T)=%d token_num=%d max_inter_size_=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                sizeof(T),token_num,max_inter_size_);
        }
                                    ...
                                    const int    seq_len)
{
    ...
    printf("[%s:%d-pid:%d-%s]: <......genericActivation.....> getActivationType()=%d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,getActivationType());
    ...
    switch (getActivationType()) {
        ...
        case ActivationType::GeGLU:
            if (inter_buf_2_ == nullptr && int8_mode_ <= 1) {
                printf("[%s:%d-pid:%d-%s]: <......invokeAddBiasGeluV2.....> inter_buf_=%p, bias1=%p, ia3_tasks=%p, ia3_weights=%p, padding_offset=%p %d %d %d\n", 
                    __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                    inter_buf_, bias1, ia3_tasks, ia3_weights, padding_offset,
                    seq_len, m, inter_size_);
                invokeAddBiasGeluV2(
        ...
        case ActivationType::ReGLU:
            printf("[%s:%d-pid:%d-%s]: <......INVOKE_GENERIC_ACT(ReluActivation).....> inter_buf_=%p, bias1=%p, ia3_tasks=%p, ia3_weights=%p, padding_offset=%p %d %d %d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                inter_buf_, bias1, ia3_tasks, ia3_weights, padding_offset,
                seq_len, m, inter_size_);
        ...
        case ActivationType::SiGLU:
            printf("[%s:%d-pid:%d-%s]: <......INVOKE_GENERIC_ACT(SiluActivation).....> inter_buf_=%p, bias1=%p, ia3_tasks=%p, ia3_weights=%p, padding_offset=%p %d %d %d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                inter_buf_, bias1, ia3_tasks, ia3_weights, padding_offset,
                seq_len, m, inter_size_);
        ...
        case ActivationType::Identity:
            printf("[%s:%d-pid:%d-%s]: <......INVOKE_GENERIC_ACT(IdentityActivation).....> inter_buf_=%p, bias1=%p, ia3_tasks=%p, ia3_weights=%p, padding_offset=%p %d %d %d\n", 
                __FUNCTION__, __LINE__, getpid(),__FILENAME__,
                inter_buf_, bias1, ia3_tasks, ia3_weights, padding_offset,
                seq_len, m, inter_size_);
