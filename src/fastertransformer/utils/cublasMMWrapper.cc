...
void cublasMMWrapper::Gemm(cublasOperation_t transa,
                           ...
                           cublasGemmAlgo_t  algo)//18
{
    printf("[%s:%d-pid:%d-%s]: <Gemm 18> m%dn%dk%d\n",  
...
void cublasMMWrapper::Gemm(cublasOperation_t transa,
                           ...
                           const int         ldb,
                           void*             C,
                           const int         ldc)// proj 11
{
    printf("[%s:%d-pid:%d-%s]: <Gemm 11> m%dn%dk%d\n",  
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n,k);
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    Gemm(transa, transb, m, n, k, A, lda, B, ldb, C, ldc, 1.0f, 0.0f);//13
}
...
void cublasMMWrapper::Gemm(cublasOperation_t transa,
                           ...
                           float             f_beta) //13
{
    printf("[%s:%d-pid:%d-%s]: <Gemm 13> m%dn%dk%d transa=%d,transb=%d CUBLAS_OP_N=%d\n",  
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n,k,(int)transa,(int)transb,(int)CUBLAS_OP_N);
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    ...
    if (using_cublasLt) {
        ...
        printf("[%s:%d-pid:%d-%s]: <Gemm 13> A:%d*%d-%d B:%d*%d-%d C:%d*%d-%d\n",  
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        ...
        printf("[%s:%d-pid:%d-%s]: <cublasGemmEx> transa:%d transb=%d m%dn%dk%d, lda=%d ldb=%d ldc=%d\n",  
            __FUNCTION__, __LINE__, getpid(),__FILENAME__,
            transa,
            transb,
            m,
...
void cublasMMWrapper::Gemm(cublasOperation_t transa,
                           ...
                           const int         ldc)//12
{
    printf("[%s:%d-pid:%d-%s]: <Gemm 12> m%dn%dk%d\n",  
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n,k);
...
void cublasMMWrapper::stridedBatchedGemm(cublasOperation_t transa,
                                         ...
                                         const float       f_alpha,
                                         const float       f_beta)//17
{
    half h_alpha = (half)f_alpha;
    half h_beta  = (half)f_beta;

    printf("[%s:%d-pid:%d-%s]: <......begin.....17> m=%d m=%d k=%d batch_count=%d, A(%d,%d) B(%d,%d) C(%d,%d)\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n, k,batch_count,
        lda,strideA,
        ldb,strideB,
...
void cublasMMWrapper::stridedBatchedGemm(cublasOperation_t transa,
                                         ...
                                         const int64_t     strideC,
                                         const int         batch_count,
                                         cudaDataType_t    computeType)//21
{
    printf("[%s:%d-pid:%d-%s]: <......begin.....21> m=%d m=%d k=%d batch_count=%d, A(%d,%d) B(%d,%d) C(%d,%d)\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n, k,batch_count,
        lda,strideA,
        ldb,strideB,
...
void cublasMMWrapper::batchedGemm(cublasOperation_t  transa,
                                  ...
                                  const int          batch_count)
{
    printf("[%s:%d-pid:%d-%s]: <......begin.....> m=%d m=%d k=%d batch_count=%d, A(%d) B(%d) C(%d)\n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n, k,batch_count,
        lda,
        ldb,
...
{
    printf("[%s:%d-pid:%d-%s]: <......begin.....> m=%d m=%d k=%d \n", 
        __FUNCTION__, __LINE__, getpid(),__FILENAME__,
        m, n, k);
