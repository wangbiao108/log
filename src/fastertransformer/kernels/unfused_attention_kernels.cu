                               ...
                               const float qk_scale)
{
    ...
    for (int qi = blockIdx.x; qi < q_length; qi += gridDim.x) {
        ...
        for (int i = 0; blockDim.x * i + threadIdx.x < k_length; i++) {
            ...
            printf("softmax_kernel:%f",mask_val);
                                  ...
                                  const T   qk_scale)
{
    ...
    for (int qi = blockIdx.x; qi < q_length; qi += gridDim.x) {
        ...
        for (int i = 0; blockDim.x * i + threadIdx.x < (k_length / 2) && i < ITEMS_PER_THREAD; i++) {
            ...
            //printf("softmax_kernel_h2:%d",(int32_t)mask_val);
                                     ...
                                     const T   scalar)
{
    ...
    // Loop over q dimension.
    //printf("++++softmax_kernel_h2_v2++++blockIdx=%d, q_length=%d", blockIdx, q_length);
    for (int qi = blockIdx.x; qi < q_length; qi += gridDim.x * Q_ITEMS_PER_THREAD) {
        //printf("qi=%d, q_length=%d ", qi, q_length);
...
#pragma unroll
        ...
        //printf("softmax_kernel_h2_v2:%d < %d, 0 < %d",threadIdx.x, k_length / 2, K_ITEMS_PER_THREAD);
        for (int i = 0; blockDim.x * i + threadIdx.x < k_length / 2 && i < K_ITEMS_PER_THREAD; ++i) {
            // The half of the index of k dimension. We will use the elements at {2 * ki, 2 * ki + 1}.
            // printf("i:%d %d Q_ITEMS=%d ",i,k_length,Q_ITEMS);
            int ki = blockDim.x * i + threadIdx.x;
...
#pragma unroll
            for (int j = 0; j < Q_ITEMS; j++) {
                ...
                //     printf("j%d-%d ", j, 1);
                // }
                printf("j%d-%d ", j, (int)attn_mask[j]);
            }
...
#define LAUNCH_MAKSED_SOFTMAX_(T_, ITEMS_PER_THREAD)                                                                   \
    ...
    if (is_half2) {                                                                                                    \
        if (grid.x % 4 == 0) {                                                                                         \
            grid.x /= 4;                                                                                               \
            printf("softmax_kernel_h2_v2-begin grid[%d,%d] block[%d,%d]",grid.x,grid.y,block.x,block.y); softmax_kernel_h2_v2<T_, ITEMS_PER_THREAD, 4>                                                              \
                <<<grid, block, 0, stream>>>((T_*)param.attention_score,                                               \
                                             ...
                                             param.q_length,                                                           \
                                             param.k_length,                                                           \
                                             (const T_)param.qk_scale);  printf("softmax_kernel_h2_v2-end ");                                              \
        }                                                                                                              \
        else {                                                                                                         \
            printf("softmax_kernel_h2"); softmax_kernel_h2<T_, ITEMS_PER_THREAD><<<grid, block, 0, stream>>>((T_*)param.attention_score,            \
                                                                                (const T_*)param.qk,                   \
                                                                                (const T_*)param.attention_mask,       \
                                                                                (const T_*)param.linear_bias_slopes,   \
                                                                                param.batch_size,                      \
        ...
        printf("softmax_kernel"); softmax_kernel<T, T_IN, ITEMS_PER_THREAD><<<grid, block, 0, stream>>>(param.attention_score,                   \
                                                                              param.qk,                                \
...
void invokeMaskedSoftmax(MaskedSoftmaxParam<T, T_IN>& param, cudaStream_t stream)
{
    ...
    dim3 grid(param.q_length, param.batch_size, param.num_heads);//8 1 16
    ...
    dim3 block((param.k_length / (is_half2 ? 2 : 1) + 31) / 32 * 32);//8
...
template<typename T>
__global__ void transpose_remove_padding(const T*     src,  //[QK*V] [1,16,64,8]
                                         T*           dst,  //[8,1024]
                                         ...
                                         const int    int8_mode)
{
    ...
    const int dst_seq_id = bid;
                                //src_batch_id*8*1024
                                //src_batch_id*8*16*64+src_seq_id*64
    const int src_offset_base = src_batch_id * seq_len * head_num * size_per_head + src_seq_id * size_per_head;
    const int dst_offset_base = dst_seq_id * head_num * size_per_head;//dst_seq_id*16*64
...
// clang-format off
template<typename T>
void invokeTransposeAttentionOutRemovePadding(T*           src,//[1,16,64,8]
                                              T*           dst,//[1,8,1024]
                                              const int    valid_word_num,//8
                                              const int    batch_size,//1
                                              const int    seq_len,//8
                                              const int    head_num,//16
                                              const int    size_per_head,//64
...
__global__ void add_fusedQKV_bias_transpose_kernel(T*                               q_buf,
                                                   ...
                                                   const bool neox_rotary_style)//0
{
