...
template<typename T, bool OUTPUT_ID, int PROMPT_SRC>  //OUTPUT_ID=true, PROMPT_SRC=0
...
#pragma unroll 1
                ...
                //running
                const int seq_id   = index % max_length;//列是输入tonken个数
                const int batch_id = index / max_length;//行是batch
                if (seq_id < length) {
                    output_ids[seq_id * batch_size + batch_id] = input_ids[index];// output_ids是seq_max_len*batch_size二维排列方式
                    /*
                                       | (seq_idx_0  , batch_0) (seq_idx_0  , batch_1) ... (seq_idx_0  , batch_n) |
                                       | (seq_idx_1  , batch_0) (seq_idx_1  , batch_1) ... (seq_idx_1  , batch_n) |
                         output_ids =  |        ... ...                                                           |
                                       | (seq_idx_k-1, batch_0) (seq_idx_k-1, batch_1) ... (seq_idx_k-1, batch_n) |
        ...
        // embedding [batch, length, hidden] [1, 8, 1024]
        ...
        const int step            = start_step + word_index % length;//start_step使用
        ...
        if (PROMPT_SRC > 0 && prompt_id >= 0) {
            ...
            embedding = embedding_table[input_id * hidden_units + col_index];//嵌入 50304*1024
        }
        T pos_embed        = pos_table == nullptr ? (T)0.f : pos_table[(step - 1) * hidden_units + col_index];//位置 8*50304
        from_tensor[index] = embedding + pos_embed;//位置和编码值取和
    }
}
                                              ...
                                              cudaStream_t          stream)
{
    ...
    //0 --> no prompts, 1 --> from loaded prompts, 2 --> from request prompts
    if (has_output_ids) {
        if (prompt_param.use_request_p_prompt_embedding) {//是否使用请求提示嵌入
            printf("invokeInputIdsEmbeddingLookupPosEncoding: 1\n");
            WORD_POS_EMBEDDING_LOOPUP_KERNEL(true, 2);//来自请求提示
        }
        else if (prompt_param.p_prompt_tuning_batch_weights != nullptr) {
            printf("invokeInputIdsEmbeddingLookupPosEncoding: 2\n");
            WORD_POS_EMBEDDING_LOOPUP_KERNEL(true, 1);//来自加载提示
        }
        else {
            printf("invokeInputIdsEmbeddingLookupPosEncoding: 3\n");
            WORD_POS_EMBEDDING_LOOPUP_KERNEL(true, 0);//无提示
        }
    }
    else {
        if (prompt_param.use_request_p_prompt_embedding) {
            printf("invokeInputIdsEmbeddingLookupPosEncoding: 4\n");
            WORD_POS_EMBEDDING_LOOPUP_KERNEL(false, 2);
        }
        else if (prompt_param.p_prompt_tuning_batch_weights != nullptr) {
            printf("invokeInputIdsEmbeddingLookupPosEncoding: 5\n");
            WORD_POS_EMBEDDING_LOOPUP_KERNEL(false, 1);
        }
        else {
            printf("invokeInputIdsEmbeddingLookupPosEncoding: 6\n");
...
template<typename T>
void invokeLookupHiddenStateOfLastToken(T*           from_tensor,//out
                                        const T*     hidden_state,
                                        const int*   input_lengths,
                                        const int    max_input_length,
                                        const int    batch_size,
...
{
    if (threadIdx.x == 0) {
        printf("----invokeTileGptPromptInputs: blockIdx.x:%d gridDim.y:%d blockIdx.y:%d input_lengths=%d\n",
            blockIdx.x, gridDim.y, blockIdx.y, input_lengths[blockIdx.x]);
        tiled_input_lengths[blockIdx.x * gridDim.y + blockIdx.y] = input_lengths[blockIdx.x];//8
        if (PREFIX_PROMPT) {
            tiled_prompt_lengths[blockIdx.x * gridDim.y + blockIdx.y] = prefix_prompt_lengths[blockIdx.x];
        }
    }

    printf("invokeTileGptPromptInputs: threadIdx.x:%d max_input_length:%d blockIdx.y:%d blockDim.x=%d\n",
            threadIdx.x,max_input_length, blockIdx.y, blockDim.x);
    for (int index = threadIdx.x; index < max_input_length; index += blockDim.x) {
        printf("++++invokeTileGptPromptInputs: index=%d dst_idx=%d src_idx=%d input_ids=%d\n",
            index,
            (blockIdx.x * gridDim.y + blockIdx.y) * max_input_length + index,
            blockIdx.x * max_input_length + index,
...
void invokeTileGptPromptInputs(int*         tiled_input_ids,
                               ...
                               cudaStream_t stream)
{
    printf("-------------invokeTileGptPromptInputs\n");
    dim3 grid(batch_size, beam_width);//(1,1)
    dim3 block(min(1024, max_input_length));//8
    if (prefix_prompt_lengths != nullptr) {
        ...
        printf("-------------invokeTileGptPromptInputs false\n");
...
void setSeqLimitLen(uint32_t* seq_len_d, Tensor seq_len, int limit_len_offset, int batch_size)
{
    ...
    for (int i = 0; i < batch_size; i++) {
        ...
        printf("seq_len_h[%d]=%d\n", i, seq_len_h[i]);
    }
    cudaH2Dcpy(seq_len_d, seq_len_h.data(), batch_size);// dst src size
}
