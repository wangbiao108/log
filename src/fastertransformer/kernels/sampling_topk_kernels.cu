                             ...
                             const bool*    skip_decode)//18
{
    ...
    if (workspace == nullptr) {
        ...
        printf("-----invokeBatchTopKSampling: workspace_size=%d, sizeof(T)=%d, temp_log_probs_buf_size=%d,sizeof(int)=%d topk_tmp_ids_buf_size=%d topk_tmp_val_buf_size=%d\n", 
            workspace_size,sizeof(T),temp_log_probs_buf_size,
            sizeof(int), topk_tmp_ids_buf_size, topk_tmp_val_buf_size);
        return;
    }
    ...
    switch (max_top_k) {
        CASE_K(1, 16, 128, 128, 8);//K_MIN, K_MAX, BLOCK_SIZE_1_, BLOCK_SIZE_2_, BLOCKS_PER_BEAM_
