...
__global__ void getPaddingOffsetAndCuSeqLensKernel(size_t*    h_valid_word_num,//out:host
                                                   int*       tmp_mask_offset,//out:dev
                                                   int*       cu_seqlens,//out:dev
                                                   const int* sequence_length,//in
                                                   ...
                                                   const int  max_seq_len)
{
    ...
    for (int i = 0; i < batch_size; i++) {
        const int seq_len = sequence_length[i];//输入seq的长度
...
template<typename T>
__global__ void remove_padding(T* tgt, const T* src, const int* padding_offset, const int n) //n: hidden_unit
{
    const int tid        = threadIdx.x;//0~255
    const int bid        = blockIdx.x;//token_id:0~(batch_size*seq_en-1)
